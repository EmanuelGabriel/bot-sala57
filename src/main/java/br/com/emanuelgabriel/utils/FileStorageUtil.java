 ...
 * that the file is read and written in a structured manner using the Gson library.
 * </p>
 * 
 * <p>
...
public class FileStorageUtil {
   ...
   * 
   * @return Um conjunto de IDs carregados do arquivo ou um conjunto vazio se o
   *         arquivo não existir.
   */
  public static Set<String> carregarIds() {
    try (Reader reader = Files.newBufferedReader(Paths.get(IDS_FILE_PATH))) {
      ...
      System.err.println("Erro ao carregar IDs do arquivo ou arquivo não encontrado. Criando novo conjunto.");
      return new HashSet<>();
    }
  }
  ...
  public static void salvarIds(Set<String> ids) {
    try (Writer writer = Files.newBufferedWriter(Paths.get(IDS_FILE_PATH))) {
      ...
      System.err.println("Erro ao salvar IDs no arquivo: " + e.getMessage());
    }
  }
